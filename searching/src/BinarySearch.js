...
const BinarySearch = async (arrayElements, target, speed) => {
  ...
  while (low <= high) {
    ...
    moveTo(".checking-element", `.array-element-${mid}`, speed, "compare");
    moveTo(".mid-1", `.array-element-${mid}`, speed, "compareKey", 0);
    if (low === mid) moveTo(".low", `.array-element-${mid}`, speed, "compareKey", 0);
    if (high === mid) moveTo(".high", `.array-element-${mid}`, speed, "compareKey", 0);
    if (low === high) {
    ...
    if (arrayElements[mid].props.value === target) {
      for (let index = 0, count = 0; index < mid; index++) setTimeout(() => document.querySelector(`.array-element-${index}`).classList.add("grey-out-binary"), count++ * 10);
      for (let index = arrayElements.length - 1, count = 0; index > mid; index--) setTimeout(() => document.querySelector(`.array-element-${index}`).classList.add("grey-out-binary"), count++ * 10);
      document.querySelector(`.array-element-${mid}`).classList.add("found");
      showOutput("found", mid);
      moveTo(".checking-element", `.array-element-${mid}`, speed, "merge");
      document.querySelector(".low").style.opacity = 0;
      document.querySelector(".high").style.opacity = 0;
